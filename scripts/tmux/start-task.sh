...
[[ $# -eq 2 ]] || die "usage: start-task.sh --config <file> [--agent <claude|codex|gemini>] [--mode shell|prompt] [--handoff-file <path>] [--goal <text>] [--in-scope <text>] [--out-of-scope <text>] [--done <text>] [--reference <path>] [--review-focus <text>] <target> <slug>"
